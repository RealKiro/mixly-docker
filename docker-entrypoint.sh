#!/bin/sh
# Mixly 服务端容器入口：检测官方运行包 -> 修正权限 -> 以非 root 启动 mixio
set -e

SERVER_DIR="/opt/mixly_server"
BIN="$SERVER_DIR/mixio/mixio"
...
==========================================================================
 ...
 请按以下步骤放置运行包（首次运行 / 升级包缺失时）：
   ...
   1. 从官方百度网盘下载 mixly_server 压缩包
      （下载地址见项目 README-docker.md，或向 mixly.cn 官方获取）
...
mkdir -p "$SERVER_DIR/mixio/storage" "$SERVER_DIR/mixio/store" "$SERVER_DIR/mixio/logs"
chown -R mixly:mixly "$SERVER_DIR/mixio/storage" "$SERVER_DIR/mixio/store" "$SERVER_DIR/mixio/logs" 2>/dev/null || true
